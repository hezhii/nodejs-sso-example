...
router.get('/', function (req, res, next) {
  ...
  if (user) {
    ...
    if (!token) {
      res.redirect(`http://passport.com:8080/login?redirectUrl=${req.headers.host + req.originalUrl}`);
    } else {
      request(
        `http://passport.com:8080/check_token?token=${token}&t=${new Date().getTime()}`,
